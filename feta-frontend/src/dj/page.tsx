...
import { Clock, DollarSign, User, CheckCircle, XCircle, Loader2 } from "lucide-react"
...
export default function SesijaPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<"all" | "pending" | "allowed" | "played" | "paid" | "rejected">(
    "all",
  )
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/narudzbe/posesiji/${sesija_id}`)
    ...
    <div className="flex flex-col min-h-screen bg-[#0B132B] text-white">
      ...
      <div className="p-4 bg-[#1C2541] border-b border-[#3A506B]">
        <div className="max-w-2xl mx-auto">
          <h2 className="text-2xl font-bold text-[#6FFFE9]">Sesija #{sesija.sesija_id}</h2>
          <div className="space-y-2 mt-2">
            <p>
              <strong>Lokacija ID:</strong> {sesija.lokacija_id}
            </p>
            <p>
              <strong>Istječe:</strong> {new Date(sesija.expiration).toLocaleString()}
            </p>
            <p>
      ...
      <div className="p-4 flex justify-center space-x-2 border-b border-[#3A506B]">
        {(["all", "pending", "allowed", "played", "paid", "rejected"] as const).map((filter) => (
          <Button
            key={filter}
